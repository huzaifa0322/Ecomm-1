...
function displayProducts(products) {
  ...
  products.forEach(data => {
    const card = `<div class='card HoverBigger'>
...
function displayCategoryChips(categories) {
  ...
  const allChip = `<button class="chip active" data-category="All" onclick="handleClick(this)"><span>All</span></button>`;
  chips.innerHTML += allChip;

  // Add the remaining category chip buttons
  categories.forEach(cat => {
    const chip = `<button class="chip" data-category="${cat}" onclick="handleClick(this)"><span>${cat}</span></button>`;
...
function handleClick(event) {
  ...
  var activeClassElements = document.getElementsByClassName('active');

  for (var i = 0; i < activeClassElements.length; i++) {
    var element = activeClassElements[i];
